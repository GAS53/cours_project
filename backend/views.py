...
def idea_card(request, pk): # Страница идеи
    title = "Идея"
...
""" Другие пользователи могут поставить лайк к идее """
...
def like_add(request, pk): # добавление лайка на проект через кнопку
    ...
    idea = Idea.objects.filter(pk=pk).first()
    autor = request.user
    if LikesToIdea.objects.filter(idea=idea, autor=autor):
        return HttpResponseRedirect(request.META.get("HTTP_REFERER"))
